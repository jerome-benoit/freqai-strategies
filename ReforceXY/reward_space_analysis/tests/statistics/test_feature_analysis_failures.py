...
def test_feature_analysis_missing_reward_column():
    ...
    importance_df, stats, partial_deps, model = _perform_feature_analysis(df, seed=7, skip_partial_dependence=True)
...
def test_feature_analysis_single_feature_path():
    ...
    importance_df, stats, partial_deps, model = _perform_feature_analysis(df, seed=11, skip_partial_dependence=True)
...
def test_feature_analysis_nans_present_path():
    ...
    importance_df, stats, partial_deps, model = _perform_feature_analysis(df, seed=13, skip_partial_dependence=True)
...
def test_feature_analysis_model_fitting_failure(monkeypatch):
    ...
    df = _minimal_df(50)
    importance_df, stats, partial_deps, model = _perform_feature_analysis(df, seed=21, skip_partial_dependence=True)
    assert stats["model_fitted"] is False
    assert model is None
    assert importance_df["importance_mean"].isna().all()
...
def test_feature_analysis_permutation_failure_partial_dependence(monkeypatch):
    ...
    df = _minimal_df(60)
    importance_df, stats, partial_deps, model = _perform_feature_analysis(df, seed=33, skip_partial_dependence=False)
...
def test_feature_analysis_success_partial_dependence():
    df = _minimal_df(70)
    importance_df, stats, partial_deps, model = _perform_feature_analysis(df, seed=47, skip_partial_dependence=False)
